...
func KubePreUpgradeDetector(js nats.JetStreamContext, kubeconfig *string) {
	...
	result = getResults(kubernetesAPIs, kubeconfig)
...
func getResults(kubeAPIs model.KubernetesAPIs, kubeconfig *string) *model.Result {
	...
	// 	kubeconfig = flag.String("kubeconfig", filepath.Join(home, ".kube", kubeConfigFileName), "(optional) absolute path to the kubeconfig file")
	// } else {
	...
	// this is a map[group/version/kind]struct{group/version, resourcename}
	...
	// f, err := os.Create("kubedc/rs.json")

	// for k, v := range ResourceAndGV {
	// 	rss = append(rss, rs{
	// 		k,
	// 		v,
	// 	})
	// }
	// by, _ := json.MarshalIndent(rss, "", " ")
	// f.Write(by)
	// defer f.Close()
